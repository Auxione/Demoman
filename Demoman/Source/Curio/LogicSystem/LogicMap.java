...
import Curio.LogicSystem.Controller.ItemSpawner;
import Curio.LogicSystem.Processor.Delay;
import Curio.LogicSystem.Processor.PushToSwitch;
...
public class LogicMap extends CellularMap {
	...
	public LogicMap(TileMap tileMap, ItemMap itemMap) {
	...
	public void put_DynamicWall(int x, int y, int changeid) {
		dynamicwallList.add(new DynamicWall(tileMap, x, y, changeid));
		if (console != null) {
	...
	public void sendTick(CellCoordinate cellCoordinate, boolean state) {
		if (state == true) {
			super.setTile(cellCoordinate, 0, 1);
		}
	}

	public void setState(CellCoordinate cellCoordinate, boolean state) {
		if (state == true) {
			super.setTile(cellCoordinate, 1, 1);
		}

		else if (state == false) {
			super.setTile(cellCoordinate, 1, 0);
		}
	}

	public boolean getState(CellCoordinate cellCoordinate) {
		if (super.getTile(cellCoordinate, 0) == 1) {
			return true;
		}

		else if (super.getTile(cellCoordinate, 1) == 1) {
			return true;
		}
