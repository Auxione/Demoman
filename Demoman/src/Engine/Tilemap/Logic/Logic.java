...
import Engine.Tilemap.Logic.Controller.DynamicWall;
import Engine.Tilemap.Logic.Controller.FireStarter;
import Engine.Tilemap.Logic.Controller.ItemSpawner;
import Engine.Tilemap.Logic.Processor.Delay;
import Engine.Tilemap.Logic.Processor.PushToSwitch;
...
public abstract class Logic {
	...
	public static void createLogicMap(GameManager gm, Tilemap lvl, int sx, int sy) {
		gameManager = gm;
		level = lvl;
		// coordinate system x,y
		// logicMap[][][0] is one tick.0=off , 1=on;
		// logicMap[][][1] is switch.0=off , 1=on;
		logicMap = new int[sx][sy][2];
	...
	public static void put_ItemSpawner(int x, int y, int id) {
		itemspawnerList.add(new ItemSpawner(gameManager, x, y, id));
	}
